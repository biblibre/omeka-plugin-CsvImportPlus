...
class CsvImportPlusPlugin extends Omeka_Plugin_AbstractPlugin
{
    ...
    public function hookUpgrade($args)
    {
        ...
        if (version_compare($oldVersion, '2.0', '<=')) {
            ...
            set_option('csv_import_html_elements', $this->_options['csv_import_plus_html_elements']);
            set_option('csv_import_automap_columns', $this->_options['csv_import_plus_automap_columns']);
        }
        ...
        if (version_compare($oldVersion, '2.0.3', '<=')) {
            set_option('csv_import_enclosure', CsvImportPlus_RowIterator::DEFAULT_ENCLOSURE);
            $sql = "
                ALTER TABLE `{$db->prefix}csv_import_imports`
        ...
        if (version_compare($oldVersion, '2.1.3-full', '<')) {
            $sql = "
                CREATE TABLE IF NOT EXISTS `{$db->prefix}csv_import_logs` (
                    `id` int(10) unsigned NOT NULL auto_increment,
                    `import_id` int(10) unsigned NOT NULL,
                    `priority` tinyint unsigned NOT NULL,
                    `created` timestamp DEFAULT CURRENT_TIMESTAMP,
        ...
        if (version_compare($oldVersion, '2.1.4-full', '<')) {
            $sql = "
                ALTER TABLE `{$db->prefix}csv_import_logs`
