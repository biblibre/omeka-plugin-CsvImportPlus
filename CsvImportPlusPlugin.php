...
class CsvImportPlusPlugin extends Omeka_Plugin_AbstractPlugin
{
    ...
    protected $_options = array(
        ...
        CsvImportPlus_ColumnMap_IdentifierField::IDENTIFIER_FIELD_OPTION_NAME => CsvImportPlus_ColumnMap_IdentifierField::DEFAULT_IDENTIFIER_FIELD,
        CsvImportPlus_RowIterator::COLUMN_DELIMITER_OPTION_NAME => CsvImportPlus_RowIterator::DEFAULT_COLUMN_DELIMITER,
        CsvImportPlus_RowIterator::ENCLOSURE_OPTION_NAME => CsvImportPlus_RowIterator::DEFAULT_ENCLOSURE,
        CsvImportPlus_ColumnMap_Element::ELEMENT_DELIMITER_OPTION_NAME => CsvImportPlus_ColumnMap_Element::DEFAULT_ELEMENT_DELIMITER,
        CsvImportPlus_ColumnMap_Tag::TAG_DELIMITER_OPTION_NAME => CsvImportPlus_ColumnMap_Tag::DEFAULT_TAG_DELIMITER,
        CsvImportPlus_ColumnMap_File::FILE_DELIMITER_OPTION_NAME => CsvImportPlus_ColumnMap_File::DEFAULT_FILE_DELIMITER,
        // Option used during the first step only.
        'csv_import_plus_html_elements' => FALSE,
        'csv_import_plus_automap_columns' => TRUE,
        'csv_import_plus_create_collections' => FALSE,
    ...
    public function hookInstall()
    {
        ...
        $db->query("CREATE TABLE IF NOT EXISTS `{$db->prefix}csv_import_plus_imports` (
        ...
        $db->query("CREATE TABLE IF NOT EXISTS `{$db->prefix}csv_import_plus_imported_records` (
            `id` int(10) unsigned NOT NULL auto_increment,
    ...
    public function hookUpgrade($args)
    {
        ...
        if (version_compare($oldVersion, '2.0-dev', '<=')) {
            $sql = "UPDATE `{$db->prefix}csv_import_plus_imports` SET `status` = ? WHERE `status` = ?";
            $db->query($sql, array('other_error', 'error'));
        }

        if (version_compare($oldVersion, '2.0', '<=')) {
            set_option(CsvImportPlus_RowIterator::COLUMN_DELIMITER_OPTION_NAME, CsvImportPlus_RowIterator::DEFAULT_COLUMN_DELIMITER);
            set_option(CsvImportPlus_ColumnMap_Element::ELEMENT_DELIMITER_OPTION_NAME, CsvImportPlus_ColumnMap_Element::DEFAULT_ELEMENT_DELIMITER);
            set_option(CsvImportPlus_ColumnMap_Tag::TAG_DELIMITER_OPTION_NAME, CsvImportPlus_ColumnMap_Tag::DEFAULT_TAG_DELIMITER);
            set_option(CsvImportPlus_ColumnMap_File::FILE_DELIMITER_OPTION_NAME, CsvImportPlus_ColumnMap_File::DEFAULT_FILE_DELIMITER);
            set_option('csv_import_plus_html_elements', $this->_options['csv_import_plus_html_elements']);
            set_option('csv_import_plus_automap_columns', $this->_options['csv_import_plus_automap_columns']);
        }

        if (version_compare($oldVersion, '2.0.1', '<=')) {
            $sql = "
                ALTER TABLE `{$db->prefix}csv_import_plus_imports`
        ...
        if (version_compare($oldVersion, '2.0.3', '<=')) {
            set_option(CsvImportPlus_RowIterator::ENCLOSURE_OPTION_NAME, CsvImportPlus_RowIterator::DEFAULT_ENCLOSURE);
            $sql = "
                ALTER TABLE `{$db->prefix}csv_import_plus_imports`
            ...
            $sql = "
                ALTER TABLE `{$db->prefix}csv_import_plus_imported_items`
        ...
        if (version_compare($oldVersion, '2.1.1-full', '<')) {
            ...
            $table = $db->getTable('CsvImportPlus_Import');
            ...
            $sql = "
                UPDATE `{$db->prefix}csv_import_plus_imports`
            ...
            $sql = "
                ...
                RENAME TO `{$db->prefix}csv_import_plus_imported_records`
            ";
            $db->query($sql);

            // Fill all record identifiers as Item.
            $sql = "UPDATE `{$db->prefix}csv_import_plus_imported_records` SET `record_type` = 'Item'";
            $db->query($sql);
        }
        ...
        if (version_compare($oldVersion, '2.1.4-full', '<=')) {
            $sql = "
                ALTER TABLE `{$db->CsvImportPlus_Log}`
    ...
    public function hookUninstall()
    {
        ...
        // Drop the tables.
        $sql = "DROP TABLE IF EXISTS `{$db->prefix}csv_import_plus_imports`";
        $db->query($sql);
        $sql = "DROP TABLE IF EXISTS `{$db->prefix}csv_import_plus_imported_records`";
        $db->query($sql);
